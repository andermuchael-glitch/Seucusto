...
@Composable private fun RawMaterialForm(i:RawMaterialEntity?,save:(D)->Unit,cancel:()->Unit){
 var n by remember(i?.id){mutableStateOf(i?.name?:"")};var u by remember(i?.id){mutableStateOf(i?.unit?:"UN")};var c by remember(i?.id){mutableStateOf(i?.costCents?.div(100.0)?.toString()?:"0")};var min by remember(i?.id){mutableStateOf(i?.minimumQuantity?.toString()?:"0")};var s by remember(i?.id){mutableStateOf(i?.supplier?:"")};var l by remember(i?.id){mutableStateOf(i?.location?:"")};var code by remember(i?.id){mutableStateOf(i?.code?:"")};var cat by remember(i?.id){mutableStateOf(i?.category?:"")};var notes by remember(i?.id){mutableStateOf(i?.notes?:"")};var err by remember(i?.id){mutableStateOf<String?>(null)}
 Column{OutlinedTextField(n,{n=it},label={Text("Nome *")},modifier=Modifier.fillMaxWidth());OutlinedTextField(u,{u=it},label={Text("Unidade *")},modifier=Modifier.fillMaxWidth());OutlinedTextField(c,{c=it},label={Text("Custo por unidade *")},modifier=Modifier.fillMaxWidth());OutlinedTextField(min,{min=it},label={Text("Estoque mínimo")},modifier=Modifier.fillMaxWidth());OutlinedTextField(s,{s=it},label={Text("Fornecedor")},modifier=Modifier.fillMaxWidth());OutlinedTextField(l,{l=it},label={Text("Localização")},modifier=Modifier.fillMaxWidth());OutlinedTextField(code,{code=it},label={Text("Código")},modifier=Modifier.fillMaxWidth());OutlinedTextField(cat,{cat=it},label={Text("Categoria")},modifier=Modifier.fillMaxWidth());OutlinedTextField(notes,{notes=it},label={Text("Observações")},modifier=Modifier.fillMaxWidth());err?.let{Text(it,color=MaterialTheme.colorScheme.error)};Row{TextButton(onClick=cancel){Text("CANCELAR")};Button(onClick={val cv=c.replace(",","." ).toDoubleOrNull();val mv=min.replace(",","." ).toDoubleOrNull();when{n.isBlank()->err="Informe o nome.";u.isBlank()->err="Informe a unidade.";cv==null||cv<0->err="Custo inválido.";mv==null||mv<0->err="Estoque mínimo inválido.";else->save(D(n.trim(),code.trim(),cat.trim(),u.trim(),mv,kotlin.math.round(cv*100).toLong(),s.trim(),l.trim(),notes.trim()))}}){Text("SALVAR")}}}
}